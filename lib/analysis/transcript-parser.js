...
export class TranscriptParser {
   ...
   */
  async parseTranscript(transcriptPath, useCache = true, options = {}) {
    // Check cache first
    ...
    const cli = options.cli || this.detectCliFromPath(transcriptPath);
    const events = this.parseEventsByCli(content, cli);
  ...
  detectCliFromPath(transcriptPath) {
    if (transcriptPath.includes(`${path.sep}.codex${path.sep}`)) return 'codex';
    if (transcriptPath.includes(`${path.sep}.gemini${path.sep}`)) return 'gemini';
    if (transcriptPath.includes(`${path.sep}.cursor${path.sep}`)) return 'cursor';
    return 'claude';
  }

  parseEventsByCli(content, cli) {
    if (cli === 'cursor') {
      return content
        .split('\n')
        .filter(Boolean)
