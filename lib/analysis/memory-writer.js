...
import { isVerifiedInsightEnvelope } from './insight-response-boundary.js';

function assertVerifiedInsightEnvelope(envelope) {
  if (!isVerifiedInsightEnvelope(envelope)) {
    const error = new Error('Memory writes require a verified insight response envelope.');
    error.code = 'ANALYSIS_RESPONSE_UNVERIFIED';
    throw error;
  }
}
...
export class MemoryWriter {
  ...
  async writeDailyLog(insights) {
    assertVerifiedInsightEnvelope(insights);
    const { analysisId, startTime } = insights;
    ...
    try {
      ...
      if (existing.includes(analysisId)) {
        console.log(`[MemoryWriter] Analysis ${analysisId} already in daily log`);
        return dailyLogPath;
      }
      ...
      console.log('[MemoryWriter] Wrote daily analysis entry');

      return dailyLogPath;
    } catch (err) {
      console.error('[MemoryWriter] Failed to write daily analysis entry');
      throw err;
    }
  }
...
> Autonomous session analysis and learnings`;
  ...
  formatDailyEntry(insights) {
    assertVerifiedInsightEnvelope(insights);
    const { analysisId, startTime, insights: data, metadata } = insights;
    ...
    let entry = `## Analysis ${analysisId} (${time})
...
**Activity:** ${metadata.messageCount} messages, ${metadata.filesModified} files modified`;
  ...
  async updateMemoryMd(insights) {
    ...
    try {
      ...
      console.error('[MemoryWriter] Failed to update MEMORY.md');
      return null;
    }
  }
